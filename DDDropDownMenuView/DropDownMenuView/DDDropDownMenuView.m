...
+ (instancetype)indexPathWithCol:(NSInteger)col row:(NSInteger)row {
    DDIndexPath *indexPath = [[self alloc] initWithColumn:col row:row];
    return indexPath;
}
...
@implementation DDBackgroundCellView
...
//add by xiyang
@property (nonatomic, retain) DDIndexPath *currentIndexPath; //当前选中的index

@end


@implementation DDDropDownMenuView {
...
{
    [self selectIndexPath:[DDIndexPath indexPathWithCol:0 row:0]];
}

- (void)selectIndexPath:(DDIndexPath *)indexPath triggerDelegate:(BOOL)trigger {
    ...
    if (indexPath.item < 0 ) {
        if (!_isClickHaveItemValid && _dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:indexPath.row column:indexPath.column] > 0){
            title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DDIndexPath indexPathWithCol:indexPath.column row:self.isRemainMenuTitle ? 0 : indexPath.row item:0]];
            if (trigger) {
                [_delegate menu:self didSelectRowAtIndexPath:[DDIndexPath indexPathWithCol:indexPath.column row:indexPath.row item:0]];
            }
        }else {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:
                            [DDIndexPath indexPathWithCol:indexPath.column row:self.isRemainMenuTitle ? 0 : indexPath.row]];
            if (trigger) {
...
- (void)selectIndexPath:(DDIndexPath *)indexPath {
    [self selectIndexPath:indexPath triggerDelegate:YES];
}

#pragma mark - setter
- (void)setDataSource:(id<DDDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        if (!self.isClickHaveItemValid && _dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:0 column:i]>0) {
            titleString = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DDIndexPath indexPathWithCol:i row:0]];
        }else {
            titleString =[_dataSource menu:self titleForRowAtIndexPath:[DDIndexPath indexPathWithCol:i row:0]];
        }
    ...
    lineLayer.backgroundColor = COLOR_HEX_STR(@"#D4D4D4", 1).CGColor;
    lineLayer.frame = CGRectMake(0, self.frame.size.height, SCREEN_WIDTH, 0.5);
...
- (instancetype)initWithOrigin:(CGPoint)origin width:(CGFloat)width andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        self.backgroundColor = COLOR_HEX_STR(@"#FFFFFF", 1);
        _origin = origin;
        ...
        _textColor = COLOR_HEX_STR(@"#333333", 1);
        _textSelectedColor = COLOR_HEX_STR(@"#23BED0", 1);
        _detailTextFont = [UIFont systemFontOfSize:11];
        _detailTextColor = COLOR_HEX_STR(@"#333333", 1);
        _indicatorColor = COLOR_HEX_STR(@"#333333", 1);
        _tableViewHeight = 300;
        _dropDownViewWidth = [UIScreen mainScreen].bounds.size.width;
        _isClickHaveItemValid = YES;
        _indicatorAlignType = DDIndicatorAlignTypeCloseToTitle;
        ...
        //注册CELL
        [_leftCollectionView registerClass:[DDDropDownMenuRowCollectionCell class] forCellWithReuseIdentifier:@"DDDropDownMenuRowCollectionCell"];
        ...
        _buttomImageView.image = [self imageWithColor:COLOR_HEX_STR(@"#D4D4D4", 1) size:CGSizeMake(1, 1)];
        ...
        _backGroundView.backgroundColor = COLOR_HEX_STR(@"#000000", 0.35);
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    return CGSizeMake((SCREEN_WIDTH - 2 * KImageMargin - 2 * KImageMargin)/KImageCount,KImageHeight * SCREEN_WIDTH / 375);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    DDDropDownMenuRowCollectionCell *cell =[collectionView dequeueReusableCellWithReuseIdentifier:@"DDDropDownMenuRowCollectionCell" forIndexPath:indexPath];
    if ([_dataSource menu:self titleForRowAtIndexPath:[DDIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]]) {
        if (indexPath.row == self.selectIndex) {
            cell.button.selected = YES;
        }else{
            cell.button.selected = NO;
        }
        [cell setupUI:[_dataSource menu:self titleForRowAtIndexPath:[DDIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]]];
    }
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        if (i != tapIndex) {
            [self animateIndicator:_indicators[i] Forward:NO complete:^{
                [self animateTitle:weakSelf.titles[i] show:NO complete:^{
...
- (void)layoutIndicator:(id)indicator withTitle:(CATextLayer *)title {
    ...
    if (self.indicatorAlignType == DDIndicatorAlignTypeCloseToTitle) {
        if (self.indicatorIsImageView) {
...
- (void)animateBackGroundView:(UIView *)view show:(BOOL)show complete:(void(^)(void))complete {
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            view.backgroundColor = COLOR_HEX_STR(@"#000000", 0.35);
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
...
- (void)animateTableView:(UICollectionView *)tableView show:(BOOL)show complete:(void(^)(void))complete {
    ...
    if (show) {
        ...
        CGFloat count = [_dataSource menu:self arrayForRowAtIndexPath:[DDIndexPath indexPathWithCol:_currentSelectedMenudIndex row:_currentSelectedMenudIndex]].count;

        CGFloat tableViewHeight = KImageHeight * SCREEN_WIDTH / 375 * ceil(count / KImageCount) + ceil(count / KImageCount) * KImageMargin + KImageMargin;

        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                weakSelf.leftCollectionView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, weakSelf.dropDownViewWidth/2, tableViewHeight);
            } else {
                weakSelf.leftCollectionView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, weakSelf.dropDownViewWidth, tableViewHeight);
            }
            weakSelf.buttomImageView.frame = CGRectMake(self.origin.x, CGRectGetMaxY(weakSelf.leftCollectionView.frame), weakSelf.dropDownViewWidth, 0.5);
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                weakSelf.leftCollectionView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, weakSelf.dropDownViewWidth/2, 0);
            } else {
                weakSelf.leftCollectionView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, weakSelf.dropDownViewWidth, 0);
            }
            weakSelf.buttomImageView.frame = CGRectMake(self.origin.x, CGRectGetMaxY(weakSelf.leftCollectionView.frame), weakSelf.dropDownViewWidth, 0.5);
        } completion:^(BOOL finished) {
            [weakSelf.leftCollectionView removeFromSuperview];
            [weakSelf.buttomImageView removeFromSuperview];
        }];
    }
    complete();
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_leftCollectionView == collectionView) {
        self.currentIndexPath = [DDIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row];
...
- (BOOL )confiMenuWithSelectRow:(NSInteger)row {
    ...
    if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:row column:_currentSelectedMenudIndex]> 0) {
        ...
        if (self.isClickHaveItemValid) {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[DDIndexPath indexPathWithCol:_currentSelectedMenudIndex row:row]];
        ...
        title.string = [_dataSource menu:self titleForRowAtIndexPath:
                        [DDIndexPath indexPathWithCol:_currentSelectedMenudIndex row:self.isRemainMenuTitle ? 0 : row]];
        [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftCollectionView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
            self.show = NO;
        }];
        return YES;
